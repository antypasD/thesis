...
\begin{quote}
\textbf{Publication Details:} This chapter reproduces verbatim our paper, ``Multilingual topic classification in X: Dataset and analysis'' (Dimosthenis Antypas, Asahi Ushio, Francesco Barbieri, and Jose Camacho-Collados, 2024, in \textit{Proceedings of the 2024 Conference on Empirical Methods in Natural Language Processing}, pages 20136 to 20152) (\Autoref{ch:intro:publications:partone}, \Autoref{publication:2}), with only minor modifications to presentation. It corresponds directly to the contributions outlined in \Autoref{ch:intro:contributions}, specifically \Autoref{contribution:1}, \textit{New Datasets for Topic Classification on Twitter}.

\textbf{Statement of Authorship:} The candidate, Dimosthenis Antypas, was the primary author of this work. The candidate was responsible for establishing the annotation framework and guidelines, coordinating the annotation process, collecting the data, aggregating the annotations, establishing the experimental setup, conducting the analysis, and writing the majority of the manuscript. The remaining co-authors provided supervision, methodological guidance, and assisted with reviewing and editing the manuscript.
\end{quote}
% ---------------
...
% Natural Language Processing, 2024 (\Autoref{ch:intro:publications:partone}, \Autoref{publication:2}), with only minor modifications to presentation. It corresponds directly to the contributions outlined in \Autoref{ch:intro:contributions}, specifically \Autoref{contribution:1}, \textit{New Datasets for Topic Classification on Twitter}.
...
Although the 19-topic taxonomy was originally curated for English discourse, the categories were deliberately designed to be broad and culturally inclusive (e.g., `Sports', `Diaries & Daily Life'). Furthermore, deploying a unified, standardised taxonomy across all languages enables a consistent cross-lingual evaluation and allows for the training of robust multilingual models, an objective that would be highly impractical with fragmented label sets.
...
To assess the quality of our annotation process, we report the following three annotation agreement metrics: (1) Krippendorff's Alpha (\textit{Alpha}) \cite{krippendorff2011computing}, (2) Percent Agreement  (\textit{PA}), ratio of number of agreements to the total number of annotations, and (3) Agreement between each pair of coders on at least one label (\textit{Overlap}). When comparing our results with those achieved in the {\TweetTopic} annotation, as presented in Table \ref{tab:annotators-agreement}, we can observe an overall smaller concordance 
 among coders. The highest \textit{Alpha} score observed was 0.26 in the Greek dataset, in contrast to {\TweetTopic}'s 0.34. Nevertheless, the agreement metrics remain on par with similar multi-label annotation tasks such as the datasets \textit{Affect in Tweets}, with a Fleiss' Kappa score of 0.26, \cite{mohammad2018semeval} and \textit{GoEmotions} \cite{demszky-etal-2020-goemotions}, with an \textit{Alpha} score of 0.24, noting that a random annotation process would yield an \textit{Alpha} score of 0. The reduction in annotator agreement can potentially be explained by the updated data collection strategy. Unlike {\TweetTopic}, which utilised keyword filtering to extract highly focused and categorised trending topics, {\XTOPIC} samples randomly from the general platform stream. This unfiltered approach leads to a higher volume of ambiguous, conversational, and multi-thematic posts, which introduce greater subjectivity among human coders. It is also worth noting that the pool of annotators was different, as {\TweetTopic} used Amazon Mechanical Turk\footnote{https://www.mturk.com/} while we sourced our annotators through Prolific.co.
...
\noindent{\textbf{Zero-shot (zero).}} No training data are provided. This setting aims to investigate the performance of zero-shot and unsupervised systems such as recent instruction tuning \cite{chung2024scaling} and generative \acp{LM} \cite{bubeck2023sparks} in low-resource settings.
...
\noindent  \textbf{XLM-R (xlmr)} \cite{conneau2019unsupervised}, a RoBERTa-like model trained on the CommonCrawls corpus \cite{wenzek2020ccnet} on 100 languages; and \textbf{XLM-T (xlmt)} \cite{barbieri2021xlm}, another XLM-R based model that utilises the last XLM-R checkpoint and further trains on a diverse dataset of over 1 billion tweets spanning over 30 languages.
...
\noindent  \textbf{mt0} \cite{muennighoff2022crosslingual}, a multilingual variant of the multilingual Text-to-Text Transfer Transformer model \cite{xue2020mt5}. %trained on the mC4 corpus \cite{T5}, which encompasses more than 101 languages. 
...
% Splitting the original table into two: Table 1 for English and Spanish, Table 2 for Japanese and Greek.
{
\renewcommand{\arraystretch}{0.991}
\begin{table*}[!ht]
\centering
...
\addtolength{\tabcolsep}{-0.2em}
\begin{tabular}{cc|lccccc|ccccc}
\multicolumn{2}{c|}{\multirow{4}{*}{\textbf{}}} & \multicolumn{1}{c}{\textbf{}} & \multicolumn{5}{c|}{\textbf{English}} & \multicolumn{5}{c}{\textbf{Spanish}} \\
\multicolumn{2}{c|}{} & \textbf{TweetTopic} & \checkmark &  &  & \checkmark & \checkmark & \checkmark &  &  & \checkmark & \checkmark \\
\multicolumn{2}{c|}{} & \textbf{Target} & \textbf{} & \textbf{\checkmark} & \textbf{} & \textbf{\checkmark} & \textbf{} & \textbf{} & \textbf{\checkmark} & \textbf{} & \textbf{\checkmark} & \textbf{} \\
\multicolumn{2}{c|}{} & \textbf{All} &  &  & \checkmark &  & \checkmark &  &  & \checkmark &  & \checkmark \\ \hline
\multirow{13}{*}{\rotatebox{90}{macro-F1}} & \multirow{5}{*}{\rotatebox{90}{finetuned}} & bernice & 55.9 & 42.7 & 55.2 & 58.7 & 60.3 & 52.0 & 26.6 & 51.5 & 55.8 & 55.9 \\
 &  & xlmr\_base & 47.0 & 25.1 & 45.9 & 58.0 & 57.6 & 42.4 & 11.6 & 35.1 & 48.4 & 49.1 \\
 &  & xlmr\_large & 57.2 & 51.1 & 58.7 & 60.8 & \textbf{\underline63.3} & 51.8 & 32.6 & 49.4 & 53.0 & 57.2 \\
 &  & xlmt\_base & 55.4 & 42.7 & 55.1 & 59.1 & 60.3 & 48.5 & 29.9 & 49.1 & 52.8 & 54.2 \\
 &  & xlmt\_large & 60.2 & 52.0 & 59.9 & 62.1 & 61.7 & 52.9 & 45.4 & 54.4 & 56.6 & \textbf{\underline60.0} \\ \cline{2-13} 
 & \multirow{4}{*}{\rotatebox{90}{zero}} & bloomz & \multicolumn{5}{c|}{23.4} & \multicolumn{5}{c}{15.5} \\
 &  & mt0 & \multicolumn{5}{c|}{34.7} & \multicolumn{5}{c}{29.2} \\
 &  & chat-gpt & \multicolumn{5}{c|}{44.9} & \multicolumn{5}{c}{37.2} \\
 &  & gpt-4o & \multicolumn{5}{c|}{59.1} & \multicolumn{5}{c}{52.4} \\ \cline{2-13} 
 & \multirow{4}{*}{\rotatebox{90}{few}} & bloomz & \multicolumn{5}{c|}{21.0} & \multicolumn{5}{c}{17.3} \\
 &  & mt0 & \multicolumn{5}{c|}{35.7} & \multicolumn{5}{c}{29.1} \\
 &  & chat-gpt & \multicolumn{5}{c|}{54.1} & \multicolumn{5}{c}{43.6} \\
 &  & gpt-4o & \multicolumn{5}{c|}{60.0} & \multicolumn{5}{c}{52.8} \\ \hline
\multirow{13}{*}{\rotatebox{90}{micro-F1}} & \multirow{5}{*}{\rotatebox{90}{finetuned}} & bernice & 63.5 & 63.1 & 67.6 & 67.1 & 66.8 & 64.9 & 68.2 & 71.8 & 72.5 & 72.4 \\
 &  & xlmr\_base & 57.3 & 51.9 & 62.6 & 65.0 & 64.0 & 59.5 & 57.8 & 68.5 & 69.4 & 70.3 \\
 &  & xlmr\_large & 64.4 & 66.2 & 67.5 & 67.2 & \textbf{68.8} & 65.4 & 69.2 & 71.6 & 71.7 & 72.4 \\
 &  & xlmt\_base & 63.5 & 63.5 & 66.6 & 66.9 & 66.2 & 63.3 & 68.7 & 71.7 & 72.5 & 71.5 \\
 &  & xlmt\_large & 66.3 & 66.3 & 68.7 & \textbf{68.8} & 67.8 & 67.0 & 72.5 & 73.9 & 73.9 & \textbf{\underline74.5} \\ \cline{2-13} 
 & \multirow{4}{*}{\rotatebox{90}{zero}} & bloomz & \multicolumn{5}{c|}{24.3} & \multicolumn{5}{c}{15.2} \\
 &  & mt0 & \multicolumn{5}{c|}{38.7} & \multicolumn{5}{c}{24.7} \\
 &  & chat-gpt & \multicolumn{5}{c|}{48.6} & \multicolumn{5}{c}{49.8} \\
 &  & gpt-4o & \multicolumn{5}{c|}{63.6} & \multicolumn{5}{c}{65.6} \\ \cline{2-13} 
 & \multirow{4}{*}{\rotatebox{90}{few}} & bloomz & \multicolumn{5}{c|}{23.5} & \multicolumn{5}{c}{14.6} \\
 &  & mt0 & \multicolumn{5}{c|}{38.8} & \multicolumn{5}{c}{25.2} \\
 &  & chat-gpt & \multicolumn{5}{c|}{57.2} & \multicolumn{5}{c}{54.9} \\
 &  & gpt-4o & \multicolumn{5}{c|}{63.2} & \multicolumn{5}{c}{62.3}
\end{tabular}
}
\captionsetup{format=hang,justification=justified}
\caption{F1 scores (macro \& micro average) for English and Spanish under each setting, tested in 5-fold cross validation. Fine-tuned models are evaluated on different settings depending on the used training data. \textit{TweetTopic:}  {\TweetTopic} was used for training; \textit{Target:} the respective language subset of {\XTOPIC} was used for training; \textit{All:} all language subsets of {\XTOPIC} were used. The best result for each language is bolded, and underlined scores indicate statistically significant difference with respect to the second best score.}
\label{tab:results_overall_english_spanish}
\end{table*}
...
\addtolength{\tabcolsep}{-0.2em}
\begin{tabular}{cc|lccccc|ccccc}
\multicolumn{2}{c|}{\multirow{4}{*}{\textbf{}}} & \multicolumn{1}{c}{\textbf{}} & \multicolumn{5}{c|}{\textbf{Japanese}} & \multicolumn{5}{c}{\textbf{Greek}} \\
\multicolumn{2}{c|}{} & \textbf{TweetTopic} & \checkmark &  &  & \checkmark & \checkmark & \checkmark &  &  & \checkmark & \checkmark \\
\multicolumn{2}{c|}{} & \textbf{Target} & \textbf{} & \textbf{\checkmark} & \textbf{} & \textbf{\checkmark} & \textbf{} & \textbf{} & \textbf{\checkmark} & \textbf{} & \textbf{\checkmark} & \textbf{} \\
\multicolumn{2}{c|}{} & \textbf{All} &  &  & \checkmark &  & \checkmark &  &  & \checkmark &  & \checkmark \\ \hline
\multirow{13}{*}{\rotatebox{90}{macro-F1}} & \multirow{5}{*}{\rotatebox{90}{finetuned}} & bernice & 45.8 & 39.9 & 55.2 & 53.3 & 54.3 & 41.4 & 26.4 & 40.1 & 43.4 & 44.0 \\
 &  & xlmr\_base & 34.4 & 2.7 & 39.9 & 50.1 & 52.5 & 29.5 & 12.3 & 34.2 & 40.0 & 39.7 \\
 &  & xlmr\_large & 49.1 & 38.5 & 55.9 & 56.6 & 56.7 & 44.0 & 26.7 & 45.6 & 45.5 & 46.2 \\
 &  & xlmt\_base & 47.8 & 29.5 & 50.8 & 53.1 & 54.4 & 32.6 & 21.8 & 39.6 & 41.3 & 45.4 \\
 &  & xlmt\_large & 50.9 & 50.9 & 57.3 & 57.2 & \textbf{\underline58.5} & 40.6 & 30.1 & 49.3 & 48.6 & 50.3 \\ \cline{2-13} 
 & \multirow{4}{*}{\rotatebox{90}{zero}} & bloomz & \multicolumn{5}{c|}{15.2} & \multicolumn{5}{c}{1.5} \\
 &  & mt0 & \multicolumn{5}{c|}{37.3} & \multicolumn{5}{c}{24.7} \\
 &  & chat-gpt & \multicolumn{5}{c|}{35.6} & \multicolumn{5}{c}{33.2} \\
 &  & gpt-4o & \multicolumn{5}{c|}{51.9} & \multicolumn{5}{c}{49.5} \\ \cline{2-13} 
 & \multirow{4}{*}{\rotatebox{90}{few}} & bloomz & \multicolumn{5}{c|}{14.0} & \multicolumn{5}{c}{5.2} \\
 &  & mt0 & \multicolumn{5}{c|}{39.0} & \multicolumn{5}{c}{25.1} \\
 &  & chat-gpt & \multicolumn{5}{c|}{43.9} & \multicolumn{5}{c}{39.5} \\
 &  & gpt-4o & \multicolumn{5}{c|}{53.3} & \multicolumn{5}{c}{\textbf{51.0}} \\ \hline
\multirow{13}{*}{\rotatebox{90}{micro-F1}} & \multirow{5}{*}{\rotatebox{90}{finetuned}} & bernice & 52.8 & 55.3 & 59.7 & 59.9 & 59.3 & 64.4 & 68.6 & 71.1 & 71.9 & 70.8 \\
 &  & xlmr\_base & 43.8 & 20.1 & 52.7 & 55.8 & 56.4 & 53.8 & 60.3 & 68.1 & 69.0 & 67.8 \\
 &  & xlmr\_large & 52.3 & 52.5 & 59.6 & 59.2 & 58.6 & 64.4 & 68.7 & 72.6 & 72.1 & 71.2 \\
 &  & xlmt\_base & 51.8 & 49.5 & 57.8 & 57.5 & 58.7 & 58.5 & 67.0 & 70.0 & 69.8 & 70.1 \\
 &  & xlmt\_large & 56.0 & 59.6 & \textbf{\underline61.4} & 60.5 & 61.3 & 65.8 & 70.6 & \textbf{\underline74.5} & 73.0 & 73.4 \\ \cline{2-13} 
 & \multirow{4}{*}{\rotatebox{90}{zero}} & bloomz & \multicolumn{5}{c|}{19.3} & \multicolumn{5}{c}{0.7} \\
 &  & mt0 & \multicolumn{5}{c|}{42.7} & \multicolumn{5}{c}{43.2} \\
 &  & chat-gpt & \multicolumn{5}{c|}{39.2} & \multicolumn{5}{c}{46.6} \\
 &  & gpt-4o & \multicolumn{5}{c|}{56.6} & \multicolumn{5}{c}{65.1} \\ \cline{2-13} 
 & \multirow{4}{*}{\rotatebox{90}{few}} & bloomz & \multicolumn{5}{c|}{17.4} & \multicolumn{5}{c}{4.4} \\
 &  & mt0 & \multicolumn{5}{c|}{41.8} & \multicolumn{5}{c}{45.5} \\
 &  & chat-gpt & \multicolumn{5}{c|}{44.3} & \multicolumn{5}{c}{53.9} \\
 &  & gpt-4o & \multicolumn{5}{c|}{57.8} & \multicolumn{5}{c}{68.6}
\end{tabular}
}
\captionsetup{format=hang,justification=justified}
\caption{F1 scores (macro \& micro average) for Japanese and Greek under each setting, tested in 5-fold cross validation. Fine-tuned models are evaluated on different settings depending on the used training data. \textit{TweetTopic:}  {\TweetTopic} was used for training; \textit{Target:} the respective language subset of {\XTOPIC} was used for training; \textit{All:} all language subsets of {\XTOPIC} were used. The best result for each language is bolded, and underlined scores indicate statistically significant difference with respect to the second best score.}
\label{tab:results_overall_japanese_greek}
\end{table*}
}
...
\label{sec:results}
The average macro and micro F1 scores for each model tested across various settings are presented in Tables \ref{tab:results_overall_english_spanish} \& \ref{tab:results_overall_japanese_greek}. Overall, the task presents a challenge for the tested models, with the top-performing classifier, \textit{xlmt-large}, achieving an average performance of 57.6\% macro-F1 when trained on all available data ({\TweetTopic} and {\XTOPIC}). The majority of models demonstrate better micro-F1 scores, as they are not penalised as heavily for errors in less frequent topics. 
...
% \label{sec:ethics}
We acknowledge the importance of the ACL Code of Ethics, and we were committed to following the guidelines in the proposed task. Given that our task included user generated content we were committed to respect the privacy of the users, by replacing each user mention in the texts with a placeholder. 

We also made sure to fairly treat the annotators who labelled the dataset, by 1) fairly compensating them with an average of £8 per hour; and 2) not sharing or storing their personal information. Overall, the total time of annotation was approximately 180 hours with a median time of 25 minutes for each ``batch'' of 50 tweets and each batch requiring 5 coders.
...
{\XTOPIC}  is shared under the CC BY-NC 4.0 Deed (Attribution-NonCommercial 4.0 International).
