...
\ac{NLP} techniques, along with specialized computational models, have long been used to process, manipulate, and extract insights from textual data \cite{jones1994natural,cambria2014jumping}. Early work focused on techniques such as dictionary or rule-based approaches, as well as traditional \ac{ML} models like \acp{SVM}, regression models, and decision trees, which often utilized \ac{BoW} approaches to represent textual features. However, with the advent of \acp{PLM} such as \textit{BERT} \cite{DBLP:journals/corr/abs-1810-04805} and more recently \acp{LLM} like \textit{GPT-3} \cite{GPT3}, the landscape of text analysis has been revolutionized. These models have dramatically enhanced our ability to capture contextual and semantic relationships, and linguistic patterns in text, enabling more sophisticated and accurate extraction of insights from diverse textual sources. \todo[]{rethink last sentence. also maybe add a reference?}

Independently on the level of sophistication of the tehcnique used, NLP researchesrs have tried to solve tasks such as sentiment analysis \cite{hutto2014vader}, hate speech detection \cite{gitari2015lexicon}, and topic classification \cite{zhang2015character} among others. Even though these tasks have been studied for a long time, the advent of social media has brought new challenges and opportunities for NLP research. With the introduction of larger models the same tasks are reframed in a more challenging way for example searching for the target of sentiment or hate speech instead of only identfying the polarity of the text.
...
\todo{don't focus only on misinformation}
However, the proliferation of social media platforms has created unprecedented challenges in political discourse and misinformation spread. Political polarization and the rapid dissemination of false information have become major societal concerns, affecting democratic processes and public opinion formation \cite{allcott2017social}. These challenges are further complicated by the heavily unstructured nature of social media content, which can pose difficulties even for larger models \cite{eisenstein2013bad,gain-etal-2023-reference}. Furthermore, it is crucial to recognise both the potential biases in existing models, tools, and datasets, as well as the environmental impact associated with developing and deploying \acp{LLM} \cite{bender2021dangers}.
...
\textbf{Ethics.} The research included in the thesis has been approved by Cardiff University's ethics comitee and also follows the guidelines of ACL code of ethics\footnote{https://www.acm.org/code-of-ethics}.

\textbf{Models.} In some chapters\todo{which chapters}, the models presented are described as \ac{sota}. While this statement may not hold true at the time of writing this thesis, the models were the best performing ones at the time of their respective publications.

\textbf{Annotations.} For the datasets\todo{which chapters} collected all annotators were paid at least minimum wage for their work.  

\textbf{Privacy.} In all chapters\todo{which chapters}, the privacy of the users whose data is used is protected. All data is anonymized and aggregated to ensure that no individual can be identified. Handles and URLs are removed from the data collected to prevent any potential identification of users. 

\textbf{Naming.} A lot of the data used in this thesis were collected from \textit{X} while it was known as \textit{Twitter} through the use of the now defunct academic \ac{API}. In this thesis, we refer to the platform as \textit{Twitter} to reflect the time of the data collection and research done.