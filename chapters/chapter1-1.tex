...
% --- PHASE 1 ADDITION: CITATION AND AUTHORSHIP PREAMBLE ---
\begin{quote}
\textbf{Publication Details:} This chapter reproduces verbatim our paper, ``Twitter Topic Classification'' (Dimosthenis Antypas, Asahi Ushio, Jose Camacho-Collados, Vitor Silva, Leonardo Neves, and Francesco Barbieri, 2022, in \textit{Proceedings of the 29th International Conference on Computational Linguistics}, pp. 3386–3400) (\Autoref{ch:intro:publications:partone}, \Autoref{publication:1}), with only minor modifications to presentation. It corresponds directly to the contributions outlined in \Autoref{ch:intro:contributions}, specifically \Autoref{contribution:1}, \textit{New Datasets for Topic Classification on Twitter}.

\textbf{Statement of Authorship:} The candidate, Dimosthenis Antypas, and Asahi Ushio were joint primary authors of this work. Dimosthenis Antypas was responsible for aggregating the annotations, establishing the experimental setup, conducting the analysis, and writing the majority of the manuscript. Asahi Ushio was responsible for data collection, establishing the annotation framework and guidelines, and coordinating the annotation process. The remaining co-authors provided supervision, methodological guidance, and assisted with reviewing and editing the manuscript.
\end{quote}
% ---------------

Social media platforms host discussions about a wide variety of topics that arise every day from breaking news and political debates to cultural trends and personal experiences. Making sense of all the content and organising it into categories is an arduous task. A common way to deal with this issue is relying on topic modelling, but topics discovered using this technique are difficult to interpret, unstable over time, and highly dependent on the characteristics of the underlying corpus. These limitations make it less suitable for applications that require consistent, well-defined topic categories—particularly in social media settings where trends evolve quickly and content is often noisy.

As discussed in \Autoref{ch:background}, there is a lack of large, high-quality, in-domain resources for supervised topic classification in social media. Such datasets are crucial for enabling the development of models that can capture the nuances of online discourse and reliably identify topics across diverse contexts and time periods. To address this gap, in this Chapter, we present a new task based on tweet topic classification in the English language and release two associated datasets. Given a wide range of topics covering the most important discussion points in social media, we provide training and testing data from recent time periods that can be used to evaluate tweet classification models. Moreover, we perform a quantitative evaluation and analysis of current general- and domain-specific \acp{LM} on the task, which provide more insights on the challenges and nature of the task. 

Finally, this Chapter forms the foundation for  \Autoref{ch:x-topic}, which extends the work to a multilingual setting. It also contributes to the {\DATASET} benchmark introduced in \Autoref{ch:super-tweeteval}, and the resources developed here are applied in \Autoref{ch:spreaders} for community profiling aimed at identifying misinformation spreaders.

%This chapter reproduces verbatim our paper \textit{Twitter Topic Classification}, published in the Proceedings of the International Conference on Computational Linguistics, 2022 (\Autoref{ch:intro:publications:partone}, \Autoref{publication:1}), with only minor modifications to presentation. It corresponds directly to the contributions outlined in \Autoref{ch:intro:contributions}, specifically \Autoref{contribution:1}, \textit{New Datasets for Topic Classification on Twitter}.
...
Similarly, solutions that use new BERT contextualized embeddings (like BERTopic \cite{bertopic}) have increased in popularity as they offer increased performance. 
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{../figures/tweet-topic/text_filtering.png}
...
In the \textit{pre-filtering}, we first removed non-English tweets by using a fastText based language identifier\footnote{\url{https://fasttext.cc/blog/2017/10/02/blog-post.html}}  \cite{bojanowski2017enriching}. %CR 
...
We opted out of a majority rule as this way our dataset can be used to develop more robust systems that can handle real-world data, which are rarely straightforward and instead can often contain complex linguistic phenomena \cite{mohammad2018semeval}. Tweets where none of the classes received at least two votes were discarded.\footnote{In total, 107 tweets were discarded during post-aggregation for failing to meet the "at least two votes" threshold.} The number of tweets in each process is summarized in Table~\ref{tab:num-tweets}.
...
We established two classification settings: (1) multi-label and (2) single-label. Sample instances from both settings are displayed in Table \ref{table:tweet_examples}.\footnote{In the dataset, usernames are replaced with the placeholder {{USER}} unless they belong to a public figure (e.g. politicians, celebrities, or government employees). For readability in this Chapter, tweet examples have been slightly modified by removing links and following these anonymisation conventions.} With this distinction, we aim to provide flexibility to users, and increase the usability of the dataset for settings and analyses, where a more fine-grained classification of tweets is not required (i.e. single-label).
...
\begin{figure}[t]
    \centering
    \scalebox{0.8}{
    \includegraphics[width=\textwidth]{../figures/tweet-topic/class_distribution.png}}
...
\begin{figure}[t]
    \centering
    \scalebox{0.8}{
    \includegraphics[width=\textwidth]{../figures/tweet-topic/class_distribution_multiclass.png}}
...
\begin{tabular}[c]{@{}l@{}}arts \& \\ culture\end{tabular} & 166.9 \textpm 67.5 & 6.5 \textpm 3.4 & 0.2 \textpm 0.6 & 0.8 \textpm 1.4 & 0.4 \textpm 0.5 & 0.1 \textpm 0.3 & 140.9 & 577 \\ \hline
\begin{tabular}[c]{@{}l@{}}business \& \\ entrepreneurs\end{tabular}  & 186.3 \textpm 65.5 & 6.4 \textpm 3.1 & 0.1 \textpm 0.2 & 0.6 \textpm 1.1 & 0.5 \textpm 0.5 & 0.0 \textpm 0.2 & 159.0 & 554 \\ \hline
\begin{tabular}[c]{@{}l@{}}celebrity \& \\ pop culture\end{tabular}  & 155.5 \textpm 67.8 & 7.4 \textpm 3.7 & 0.2 \textpm 0.9 & 0.6 \textpm 1.0 & 0.8 \textpm 0.7 & 0.1 \textpm 0.4 & 145.8 & 1685 \\ \hline
\begin{tabular}[c]{@{}l@{}}diaries \& \\ daily life\end{tabular}         & 168.3 \textpm 68.4 & 5.4 \textpm 3.3 & 0.1 \textpm 0.7 & 0.4 \textpm 0.9 & 0.4 \textpm 0.5 & 0.1 \textpm 0.5 & 132.5 & 1525 \\ \hline
\begin{tabular}[c]{@{}l@{}} family  \\ ~ \end{tabular}  & 165.1 \textpm 68.5 & 5.2 \textpm 3.2 & 0.2 \textpm 1.4 & 0.5 \textpm 1.0 & 0.4 \textpm 0.5 & 0.2 \textpm 0.5 & 112.7 & 358 \\ \hline
\begin{tabular}[c]{@{}l@{}}fashion \& \\ style\end{tabular} & 147.9 \textpm 55.4 & 7.8 \textpm 3.1 & 0.2 \textpm 0.5 & 1.0 \textpm 1.5 & 0.6 \textpm 0.5 & 0.1 \textpm 0.3 & 98.8  & 251 \\ \hline
\begin{tabular}[c]{@{}l@{}}film tv \& \\ video\end{tabular} & 157.7 \textpm 66.3 & 7.5 \textpm 3.7 & 0.2 \textpm 0.8 & 0.6 \textpm 1.1 & 0.7 \textpm 0.6 & 0.1 \textpm 0.4 & 145.1 & 1723\\ \hline
\begin{tabular}[c]{@{}l@{}}fitness \& \\ health\end{tabular} & 195.4 \textpm 67.1 & 6.3 \textpm 2.8 & 0.1 \textpm 0.1 & 0.5 \textpm 0.9 & 0.6 \textpm 0.5 & 0.1 \textpm 0.3 & 168.5 & 508 \\ \hline
\begin{tabular}[c]{@{}l@{}}food \& \\ dining\end{tabular}   & 165.2 \textpm 64.5 & 6.1 \textpm 3.1 & 0.1 \textpm 0.2 & 0.5 \textpm 1.0 & 0.4 \textpm 0.5 & 0.1 \textpm 0.4 & 154.7 & 255 \\ \hline
\begin{tabular}[c]{@{}l@{}} gaming \\ ~ \end{tabular}   & 159.6 \textpm 68.9 & 6.5 \textpm 3.9 & 0.1 \textpm 0.2 & 0.5 \textpm 1.0 & 0.5 \textpm 0.6 & 0.0 \textpm 0.2 & 128.4 & 437 \\ \hline
\begin{tabular}[c]{@{}l@{}}learning \& \\ educational\end{tabular}    & 191.8 \textpm 65.8 & 5.9 \textpm 2.9 & 0.1 \textpm 0.1 & 0.6 \textpm 1.0 & 0.5 \textpm 0.6 & 0.0 \textpm 0.2 & 156.7 & 293 \\ \hline
\begin{tabular}[c]{@{}l@{}} music \\ ~ \end{tabular}  & 143.5 \textpm 64.0 & 8.4 \textpm 4.4 & 0.3 \textpm 1.1 & 0.7 \textpm 1.1 & 0.8 \textpm 0.7 & 0.1 \textpm 0.5 & 119.8 & 1919\\ \hline
\begin{tabular}[c]{@{}l@{}}news \& \\social concern\end{tabular}    & 183.1 \textpm 70.5 & 6.6 \textpm 3.0 & 0.2 \textpm 1.3 & 0.4 \textpm 0.8 & 0.6 \textpm 0.6 & 0.0 \textpm 0.2 & 165.1 & 3698 \\ \hline
\begin{tabular}[c]{@{}l@{}}other \\ hobbies\end{tabular}  & 160.9 \textpm 69.2 & 6.3 \textpm 3.4 & 0.2 \textpm 0.7 & 0.6 \textpm 1.0 & 0.4 \textpm 0.6 & 0.1 \textpm 0.4 & 143.6 & 568 \\ \hline
\begin{tabular}[c]{@{}l@{}} relationships  \\ ~  \end{tabular}            & 162.4 \textpm 70.6 & 5.3 \textpm 3.5 & 0.2 \textpm 1.6 & 0.4 \textpm 0.9 & 0.5 \textpm 0.6 & 0.2 \textpm 0.9 & 111.9 & 432 \\ \hline
\begin{tabular}[c]{@{}l@{}}science \& \\ technology\end{tabular} & 177.9 \textpm 69.4 & 6.7 \textpm 2.8 & 0.1 \textpm 0.5 & 0.5 \textpm 1.0 & 0.6 \textpm 0.5 & 0.0 \textpm 0.1 & 164.2 & 542 \\ \hline
\begin{tabular}[c]{@{}l@{}}sports  \\ ~ \end{tabular}  & 162.8 \textpm 65.9 & 6.4 \textpm 3.2 & 0.2 \textpm 1.4 & 0.5 \textpm 0.8 & 0.7 \textpm 0.6 & 0.1 \textpm 0.3 & 152.8 & 2977 \\ \hline
\begin{tabular}[c]{@{}l@{}}travel \& \\ adventure\end{tabular} & 175.2 \textpm 72.3 & 6.2 \textpm 3.1 & 0.2 \textpm 1.8 & 0.5 \textpm 1.0 & 0.5 \textpm 0.5 & 0.1 \textpm 0.2 & 173.1 & 190 \\ \hline
\begin{tabular}[c]{@{}l@{}}youth \& \\ student life\end{tabular} & 202.0 \textpm 62.4 & 5.9 \textpm 3.2 & 0.1 \textpm 0.1 & 0.5 \textpm 0.9 & 0.5 \textpm 0.6 & 0.1 \textpm 0.2 & 155.6 & 174 \\ \hline
%\hline
\end{tabular}}
...
The last two columns correspond to the lexical diversity (\ac{MTLD}) and total number of tweets.}
...
\noindent \textbf{Comparison systems.} To evaluate our dataset, we first use simple baselines: Majority (most frequent class in training) and Random (uniform probability for each class). As comparison systems, we train a traditional \ac{BoW} with SVM
and a fastText classifier \cite{bojanowski2017enriching} that utilizes pretrained embeddings \cite{mikolov2018advances}. Furthermore, BERT base and large \cite{BERT} and both base and large versions of RoBERTa \cite{ROBERTA} are used as comparison systems. As classifiers specialized on social media, i.e. trained on \textit{Twitter} data, BERTweet \cite{bertweet}, TimeLM-19, and TimeLM-21 \cite{loureiro-etal-2022-timelms}, all based on a RoBERTa architecture, are also utilized. BERTweet is trained on a corpus of 845M tweets mainly from 01/2012 to 08/2019, while also including 5M COVID-19 related tweets from 01/2020 to 03/2020. On the other hand, TimeLM-19 is trained on 95M tweets gathered between 2018 and 2019. For completeness, we also report results of TimeLM-21, trained on 125M tweets from 2018 to 2021, but excluded it from our main analysis given the time overlap with the test set (reminder that one of the motivations of this task is to be able to process tweets in real time). TimeLMs models use the RoBERTa-base model as initial checkpoint, while BERTweet is trained from scratch. 

The implementations provided by Hugging Face \cite{wolf2019huggingface} are used to train and test all \acp{LM}.\footnote{More details about the exact hyperparameters are included in the Appendix \ref{appendix:TweetTopic-evaluation}.}
...
\hline
\multicolumn{1}{|c|}{\multirow{2}{*}{\textbf{Class}}} & \multicolumn{2}{c|}{\textbf{RB}} & \multicolumn{2}{c|}{\textbf{RB-large}} & \multicolumn{2}{c|}{\textbf{BERTweet}} & \multicolumn{2}{c|}{\textbf{TimeLM-19}} & \multicolumn{2}{c|}{\textbf{TimeLM-21}} \\ \cline{2-11} 
\multicolumn{1}{|c|}{} & \multicolumn{1}{l|}{\textbf{temp}} & \multicolumn{1}{l|}{\textbf{rand}} & \multicolumn{1}{c|}{\textbf{temp}} & \textbf{rand} & \multicolumn{1}{c|}{\textbf{temp}} & \textbf{rand} & \multicolumn{1}{c|}{\textbf{temp}} & \textbf{rand} & \multicolumn{1}{c|}{\textbf{temp}} & \textbf{rand} \\ \hline
\begin{tabular}[c]{@{}l@{}}arts \& \\ culture\end{tabular} & \multicolumn{1}{c|}{20.9} & \textbf{41.2} & \multicolumn{1}{c|}{28.0} & \textbf{44.0} & \multicolumn{1}{c|}{9.8} & \textbf{28.2} & \multicolumn{1}{c|}{21.3} & \textbf{39.1} & \multicolumn{1}{c|}{35.4} & \textbf{44.8} \\ \hline
\begin{tabular}[c]{@{}l@{}}business \& \\ entrepreneurs\end{tabular} & \multicolumn{1}{c|}{56.7} & \textbf{57.1} & \multicolumn{1}{c|}{50.9} & 56.5 & \multicolumn{1}{c|}{\textbf{59.7}} & 54.5 & \multicolumn{1}{c|}{\textbf{58.6}} & 55.3 & \multicolumn{1}{c|}{\textbf{56.3}} & 54.0 \\ \hline
\begin{tabular}[c]{@{}l@{}}celebrity \& \\ pop culture\end{tabular} & \multicolumn{1}{c|}{47.2} & \textbf{52.7} & \multicolumn{1}{c|}{50.5} & \textbf{59.6} & \multicolumn{1}{c|}{43.7} & \textbf{54.9} & \multicolumn{1}{c|}{\textbf{48.6}} & 47.8 & \multicolumn{1}{c|}{46.4} & \textbf{57.6} \\ \hline
\begin{tabular}[c]{@{}l@{}}diaries \& \\ daily life\end{tabular} & \multicolumn{1}{c|}{46.2} & \textbf{50.3} & \multicolumn{1}{c|}{43.5} & \textbf{49.3} & \multicolumn{1}{c|}{44.6} & \textbf{49.9} & \multicolumn{1}{c|}{44.5} & \textbf{51.2} & \multicolumn{1}{c|}{44.7} & \textbf{49.8} \\ \hline
\begin{tabular}[c]{@{}l@{}}family \\ ~\end{tabular} & \multicolumn{1}{c|}{50.6} & \textbf{56.8} & \multicolumn{1}{c|}{52.8} & \textbf{63.4} & \multicolumn{1}{c|}{46.1} & \textbf{49.1} & \multicolumn{1}{c|}{46.4} & \textbf{55.2} & \multicolumn{1}{c|}{53.1} & \textbf{56.2} \\ \hline
\begin{tabular}[c]{@{}l@{}}fashion \& \\ style\end{tabular} & \multicolumn{1}{c|}{66.4} & \textbf{74.1} & \multicolumn{1}{c|}{66.4} & \textbf{77.4} & \multicolumn{1}{c|}{56.0} & \textbf{68.8} & \multicolumn{1}{c|}{\textbf{66.4}} & 75.2 & \multicolumn{1}{c|}{67.2} & \textbf{75.2} \\ \hline
\begin{tabular}[c]{@{}l@{}}film tv \& \\ video\end{tabular} & \multicolumn{1}{c|}{64.4} & \textbf{71.4} & \multicolumn{1}{c|}{64.7} & \textbf{71.3} & \multicolumn{1}{c|}{66.8} & \textbf{69.2} & \multicolumn{1}{c|}{66.1} & \textbf{72.2} & \multicolumn{1}{c|}{65.4} & \textbf{70.6} \\ \hline
\begin{tabular}[c]{@{}l@{}}fitness \& \\ health\end{tabular} & \multicolumn{1}{c|}{52.4} & \textbf{53.2} & \multicolumn{1}{c|}{62.4} & \textbf{65.4} & \multicolumn{1}{c|}{\textbf{48.2}} & 38.7 & \multicolumn{1}{c|}{\textbf{55.7}} & 42.2 & \multicolumn{1}{c|}{58.6} & 52.6 \\ \hline
\begin{tabular}[c]{@{}l@{}}food \& \\ dining\end{tabular} & \multicolumn{1}{c|}{\textbf{75.1}} & 75.3 & \multicolumn{1}{c|}{\textbf{79.3}} & 68.2 & \multicolumn{1}{c|}{\textbf{74.5}} & 65.7 & \multicolumn{1}{c|}{\textbf{75.4}} & 70.7 & \multicolumn{1}{c|}{\textbf{80.4}} & 71.6 \\ \hline
\begin{tabular}[c]{@{}l@{}}gaming \\ ~\end{tabular} & \multicolumn{1}{c|}{58.4} & \textbf{61.4} & \multicolumn{1}{c|}{63.8} & \textbf{69.1} & \multicolumn{1}{c|}{66.1} & \textbf{67.6} & \multicolumn{1}{c|}{64.6} & \textbf{69.2} & \multicolumn{1}{c|}{64.8} & 71.2 \\ \hline
\begin{tabular}[c]{@{}l@{}}learning \& \\ educational\end{tabular} & \multicolumn{1}{c|}{\textbf{49.5}} & 48.7 & \multicolumn{1}{c|}{49.8} & 45.8 & \multicolumn{1}{c|}{\textbf{42.9}} & 36.2 & \multicolumn{1}{c|}{\textbf{49.3}} & 47.1 & \multicolumn{1}{c|}{\textbf{48.9}} & 47.0 \\ \hline
\begin{tabular}[c]{@{}l@{}}music \\ ~\end{tabular} & \multicolumn{1}{c|}{86.0} & \textbf{87.1} & \multicolumn{1}{c|}{87.4} & \textbf{88.1} & \multicolumn{1}{c|}{86.9} & \textbf{87.2} & \multicolumn{1}{c|}{\textbf{88.1}} & 87.8 & \multicolumn{1}{c|}{86.9} & \textbf{88.2} \\ \hline
\begin{tabular}[c]{@{}l@{}}news \& \\ social concern\end{tabular} & \multicolumn{1}{c|}{83.9} & \textbf{84.6} & \multicolumn{1}{c|}{85.5} & \textbf{85.9} & \multicolumn{1}{c|}{83.5} & \textbf{84.3} & \multicolumn{1}{c|}{84.4} & \textbf{86.2} & \multicolumn{1}{c|}{84.5} & \textbf{85.0} \\ \hline
\begin{tabular}[c]{@{}l@{}}other hobbies \\ ~ \end{tabular} & \multicolumn{1}{c|}{25.0} & \textbf{28.4} & \multicolumn{1}{c|}{31.7} & \textbf{35.4} & \multicolumn{1}{c|}{\textbf{23.1}} & 21.5 & \multicolumn{1}{c|}{27.7} & \textbf{30.3} & \multicolumn{1}{c|}{31.1} & 26.2 \\ \hline
\begin{tabular}[c]{@{}l@{}}relationships \\ ~ \end{tabular} & \multicolumn{1}{c|}{37.6} & \textbf{51.8} & \multicolumn{1}{c|}{39.3} & \textbf{56.8} & \multicolumn{1}{c|}{36.8} & \textbf{51.2} & \multicolumn{1}{c|}{35.3} & \textbf{51.6} & \multicolumn{1}{c|}{44.5} & \textbf{54.0} \\ \hline
\begin{tabular}[c]{@{}l@{}}science \& \\ technology\end{tabular} & \multicolumn{1}{c|}{54.2} & \textbf{56.4} & \multicolumn{1}{c|}{52.1} & \textbf{59.4} & \multicolumn{1}{c|}{46.9} & \textbf{53.2} & \multicolumn{1}{c|}{50.5} & \textbf{56.0} & \multicolumn{1}{c|}{50.2} & \textbf{52.1} \\ \hline
\begin{tabular}[c]{@{}l@{}}sports \\ ~ \end{tabular} & \multicolumn{1}{c|}{\textbf{94.8}} & 94.2 & \multicolumn{1}{c|}{94.6} & \textbf{95.4} & \multicolumn{1}{c|}{\textbf{95.4}} & 94.4 & \multicolumn{1}{c|}{\textbf{95.6}} & 94.8 & \multicolumn{1}{c|}{\textbf{95.2}} & 94.8 \\ \hline
\begin{tabular}[c]{@{}l@{}}travel \& \\ adventure\end{tabular} & \multicolumn{1}{c|}{41.5} & \textbf{47.7} & \multicolumn{1}{c|}{46.3} & \textbf{59.9} & \multicolumn{1}{c|}{\textbf{38.5}} & 0.0 & \multicolumn{1}{c|}{\textbf{57.1}} & 56.0 & \multicolumn{1}{c|}{52.2} & 54.7 \\ \hline
\begin{tabular}[c]{@{}l@{}}youth \& \\ student life\end{tabular} & \multicolumn{1}{c|}{49.2} & \textbf{52.4} & \multicolumn{1}{c|}{21.0} & \textbf{46.0} & \multicolumn{1}{c|}{31.6} & \textbf{35.2} & \multicolumn{1}{c|}{\textbf{50.4}} & 43.6 & \multicolumn{1}{c|}{50.8} & \textbf{51.0} \\ \hline
\textbf{samples avg} & \multicolumn{1}{c|}{73.1} & \textbf{74.2} & \multicolumn{1}{c|}{74.4} & \textbf{76.4} & \multicolumn{1}{c|}{\textbf{73.8}} & 73.2 & \multicolumn{1}{c|}{74.3} & \textbf{75.2} & \multicolumn{1}{c|}{74.7} & \textbf{75.2} \\ \hline
\textbf{macro avg} & \multicolumn{1}{c|}{55.8} & \textbf{60.3} & \multicolumn{1}{c|}{56.3} & \textbf{63.0} & \multicolumn{1}{c|}{52.7} & \textbf{53.1} & \multicolumn{1}{c|}{57.2} & \textbf{59.6} & \multicolumn{1}{c|}{58.8} & \textbf{60.9} \\ \hline
\end{tabular}
}
\caption{
  Average F1 scores for the multi-label setting when using temporal (temp) and random (rand) split. Highlighted with bold is the best score for each model}
...
\begin{figure}[t]
    \centering
    \scalebox{0.8}{
    \includegraphics[width=\textwidth]{../figures/tweet-topic/performance_diff_single.png}}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{../figures/tweet-topic/troberta2019_confusion.png}
...
\label{erroranalysis}
To better understand the nature of errors made by \acp{LM}, Figure \ref{fig:confusion_matrix} shows a confusion matrix for the best-performing TimeLM-19 model in the single-label setting.
...
Despite the contributions of this work, certain limitations in the current dataset construction must be acknowledged, namely its restriction to the English language and the exclusion of tweets containing media. Our current methodology centres on scenarios where only textual data is available or prioritised. While this represents a substantial portion of standard social media analysis, it overlooks nuances in highly visual categories, such as \textit{fashion \& style} or \textit{arts \& culture}. However, the framework we present can easily be adapted and expanded in future iterations to incorporate multi-modal analysis.

While this first iteration of {\TweetTopic} focuses on English, our initial aim was to apply the same methodology to other languages. We subsequently carry this out in \Autoref{ch:x-topic}, utilising the guidelines and data construction process described in Section \ref{data} as a foundation.
